...
1. **新用户**:
   - 先阅读 [配置文档](./configuration.md) 了解如何配置
   - 然后阅读 [工作流程文档](./workflow.md) 了解工作原理
...
SSH Channels Hub 是一个用于管理和维护多个 SSH 通道的 CLI 应用程序。它通过配置文件定义需要建立的 SSH 连接，自动管理连接生命周期，并在连接断开时自动重连。

### 核心特性

- ✅ 多通道管理：同时管理多个 SSH 连接和通道
- ✅ 自动重连：连接断开时自动重连，支持多种重试策略
- ✅ 灵活配置：支持 TOML 格式配置文件
- ✅ 多种认证：支持密码、密钥、SSH Agent 认证
- ✅ 通道类型：支持 Session 和 Direct-TCPIP（端口转发）
