...
`config.toml` 默认查找顺序:`./config.toml` → `$XDG_CONFIG_HOME/ssh-channels-hub/config.toml`(未设置时使用 `~/.config/ssh-channels-hub/config.toml`) → `%APPDATA%\ssh-channels-hub\config.toml`(Windows)。也可用 `--config` 显式指定。
